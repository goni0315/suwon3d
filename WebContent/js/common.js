		...
		success : function(result){
			if(result[0].returnCode == "S"){
				...
				if(systemFalg == "main"){ //메인 시스템에서 접속할 경우
					...
					if(result[0].userInfo.aut_cde=="SYS001"){ //관리자일 경우 관리자 페이지 이동 버튼 생성 
					...
					if(result[0].userInfo.aut_cde!="SYS001"){ //관리자로 다시 한번 확인해서 권한 없으면 에러페이지로  
						alert("관리자 권한이 없습니다.");
...
function newResult(result) {
	for(var i=0;i<result.length;i++){
		...
		//alert("result[i].hide_low : " + result[i].hide_low + "///result[i].hide_high" + result[i].hide_high);            
...
// 탭메뉴
function showHistory(viewObj,chkNum,onImg) {//보일Obj ID/Obj 갯수/ //하위ID, 체크넘(탭갯수), 현재ID
...
//		var url = getMainContextPath()+"/writeLog.do?usrname="+pUsrname+"&usrid="+usrInfo.usrid+"&part="+usrInfo.part+"&deptname="+pDeptname+"&menuid="+menuid+"&menuname="+pMenuname; //요청 url
		var url = "/Suwon3d/writeLog.do?usrname="+pUsrname+"&usrid="+usrInfo.usrid+"&part="+usrInfo.part+"&deptname="+pDeptname+"&menuid="+menuid+"&menuname="+pMenuname; //요청 url
...
/**
 * 메뉴 아이디를 넣으면 현재 로그인한 유저의 권한과 비교해서
 * 권한이 있으면 콜백함수를 실행 없으면 얼럿창 밑 false반환 
 * @param menuid
 */ 
