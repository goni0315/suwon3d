...
if(dom) {
	...
	strCalendar += "<div id='calendar'>";
	strCalendar += "<table class='cnj_td'>";
	strCalendar += "	<tr bgcolor='#EEEEEE' height=20>";
	strCalendar += "		<td>";
	strCalendar += "			<table width='200' border=0>";
...
function constructMonth() {
	...
	if(!monthConstructed) { // 월 이전 월 링크
		...
		for (var i = nStartingMonth; i <= nEndMonth; i++) {
			sName =	i;
...
function constructCalendar() {
	...
	if (intWeekCount == 6)
		...
		//popHeight = 177;
		popHeight = 195;
	oPopup.show(cleft, ctop, 208, popHeight, document.body);
}
