		...
		<table border="0" cellpadding="0" cellspacing="0" class="wps_100" title="list" summary="list">
			...
			<thead>
				...
				<c:if test="${not empty roadList}">
					<c:forEach var="sms" items="${roadList}" varStatus="status">
						<tr>
							<td rowspan="2" align="center" class="f_dblue_b">${status.count}</td>
							<!-- <td class="f_orange_b"> -->
							<td class="f_orange_b" onclick="doView('KO3D_PL_BBND', '<c:out value="${sms.pnu}"></c:out>')" style="cursor: pointer;"><!--KO3D_NADD_ROAD_T //rds_man_no -->
								${sms.sgg_nm} ${sms.rn} ${sms.buld_mnnm}
								<c:if test="${sms.buld_slno!=0}">- ${sms.buld_slno}</c:if><%-- ${sms.buld_mnnm} ${sms.buld_slno} ${sms.jibun}--%>
							</td>
							<td  align="center">
						...
						<tr>
							<td>${sms.sgg_nm} ${sms.emd_nm} ${sms.lnbr_mnnm} 
