...
<body>
	<table id=popUp width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
			...
			<td valign="top" bgcolor="#FFFFFF">
				...
				<div class="bbsNext">
					<table class="wps_100" summary="이전 다음 페이지">
						...
						<tr>
							...
							<td><a onfocus=blur() href="popNoticeCount.do?bod_seq=${prevContent.bod_seq }">${prevContent.bod_sub } </a></td>
						</tr>
						</c:if>
						<c:if test="${not empty nextContent}">
						<tr>
							<th scope="row" class="bbsFrom" name="nextNotice">다음글</th>
							<td><a onfocus=blur() href="popNoticeCount.do?bod_seq=${nextContent.bod_seq }">${nextContent.bod_sub }</a></td>
						</tr>
						</c:if>
					</table>
				</div>
				<div id="bbsBottom">
					<div id="bbsBtnR">
					<!-- 목록으로 -->
						<a href="popNoticeList.do?pageNum=${pageNum}">
